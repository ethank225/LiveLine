...
const STATUS_CONFIG = {
  ...
  expired:          { border: 'border-amber-500',   bg: 'bg-amber-500/10',   label: 'Expired at market',        text: 'text-amber-400',   bar: 'bg-amber-500/70' },
...
function PositionCard({ position }) {
  ...
  return (
    <PositionShell position={position} isOpen={isOpen}>
      {isOpen ? (
        <OpenBody position={position} />
      ) : (
        <TerminalBody position={position} />
      )}
    </PositionShell>
  )
}
...
function PositionShell({ position, isOpen, children }) {
  const [faded, setFaded] = useState(false)
  const rawStatus = position.status || 'open'
  const config = STATUS_CONFIG[rawStatus] || STATUS_CONFIG.open
  ...
  const effectiveStatus = (isOpen && isPending && rawStatus === 'open')
    ? 'pending_exit'
    : rawStatus
  const shownConfig = STATUS_CONFIG[effectiveStatus] || config

  // Fade terminal positions after 30 s. A zero-delay setTimeout covers the
  // already-expired case without calling setState directly inside the effect.
  useEffect(() => {
    if (!isTerminal) return
    const completedAt = position.completed_at ? new Date(position.completed_at).getTime() : Date.now()
    const elapsed = Date.now() - completedAt
    const delay = Math.max(0, 30000 - elapsed)
    const t = setTimeout(() => setFaded(true), delay)
    return () => clearTimeout(t)
  }, [isTerminal, position.completed_at])
  ...
  return (
    <div
      className={`
        ...
        transition-[border-color,opacity] duration-700 ease-out
        ${shownConfig.border}
        ${faded ? 'opacity-50' : ''}
      `}
    >
      {/* Header row — same in every state. Age ticker self-updates. */}
      <div className="mb-1 flex items-baseline gap-2">
        ...
        <span className="text-[10px] text-slate-500 font-mono tabular-nums flex-shrink-0">
          {isTerminal
            ? formatClock(position.completed_at || position.created_at)
...
// ---------------------------------------------------------------------------
// Terminal body — result with final metrics, full bar
// ---------------------------------------------------------------------------

function TerminalBody({ position }) {
  const status = position.status || 'filled'
  const config = STATUS_CONFIG[status]
  const pnl = position.realized_pnl ?? 0
  const entry = position.entry_price
  const target = position.sell_target
  const exitPrice = position.exit_price ?? position.current_price
  const isCanceled = status === 'canceled' || status === 'canceled_by_user'

  return (
    <>
      <div className="flex items-center justify-between">
        <div className={`text-xs transition-colors duration-700 ease-out ${config.text}`}>{config.label}</div>
        {!isCanceled && (
          <span className={`text-sm font-mono font-bold tabular-nums ${pnl >= 0 ? 'text-emerald-400' : 'text-red-400'}`}>
            {pnl >= 0 ? '+' : ''}${pnl.toFixed(2)}
          </span>
        )}
      </div>
      ...
      <div className="flex items-center justify-between text-[10px] mt-auto pt-2">
        ...
        <span className="text-slate-400 font-medium">
          {status === 'stopped' ? 'Stop' : 'Sell aim'}:{' '}
          <span className="text-white font-mono">
            {formatCents(status === 'stopped' ? position.stop_loss : target)}
          </span>
        </span>
        <span className="text-slate-500">
          Exit <span className="text-slate-300 font-mono">{formatCents(exitPrice)}</span>
        </span>
      </div>

      <div className="relative h-1 bg-slate-800 rounded-full mt-1.5 overflow-hidden">
        <div
          className={`absolute inset-y-0 left-0 transition-colors duration-700 ease-out ${config.bar}`}
          style={{ width: '100%' }}
        />
      </div>
    </>
